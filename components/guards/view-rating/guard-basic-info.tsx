...
export function GuardBasicInfo({ guardData }: { guardData: IGuardMeta | null }) {
    return (
            ...
            <div className="flex items-start gap-4">
                ...
                <div className="flex-1 space-y-3">
                    ...
                    <div className="flex items-center gap-4">
                        <div className="flex items-center gap-1">
                            <span className="text-blue-600 underline cursor-pointer text-sm">
                                Reviews {guardData?.guardRatingCount || 0}
                            </span>
                        </div>

                        <div className="flex items-center gap-1">
                            {renderStars(guardData?.overallRatings || 0)}
                        </div>

                        <span className="font-bold text-lg">
                            {guardData?.overallRatings?.toFixed(1) || '0.0'}
                        </span>
                    ...
                    <div className="flex gap-3 pt-2">
                        <Button asChild>
                            <Link href={`/guards/${guardData?.guard.uuid}/add-rating`}>
                                Give Rating
                            </Link>
                        </Button>
                    </div>
